#!/usr/bin/env python3
"""
Batch process all QLD PDFs and extract to CSV files with proper column filtering.
This script will process all PDFs in data/raw/qld and save extracted data to data/processed/qld
"""
...
def process_all_qld_pdfs(input_dir=None, output_dir=None, logger=None, force_reprocess=True):
    """Process all QLD PDFs and extract to CSV files
    ...
    if input_dir is None:
        input_dir = "/Users/rorypearson/Desktop/harness_db/stew_reports/data/raw/qld"
    if output_dir is None:
        output_dir = "/Users/rorypearson/Desktop/harness_db/stew_reports/data/processed/qld"
    
    # Find all PDF files in QLD folder
    ...
    logger.info(f"Found {len(pdf_files)} QLD PDF files to process")
    ...
    for i, pdf_file in enumerate(pdf_files, 1):
        ...
        # Skip if already processed (check all format folders)
        ...
        try:
            ...
            if extracted_data and 'runners' in extracted_data and len(extracted_data['runners']) > 0:
                ...
                # Export to CSV (with QLD column filtering applied)
    ...
    elapsed = datetime.now() - start_time
    print("\n" + "=" * 80)
    print("QLD BATCH PROCESSING COMPLETE")
    print("=" * 80)
    ...
    print(f"Average rate: {processed/elapsed.total_seconds()*60:.1f} files/minute")
...
def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] in ['-h', '--help']:
            print("QLD PDF Batch Processor")
            print("Usage: python process_all_qld.py [--debug]")
            print("")
            print("Options:")
            ...
            print("- Process all PDF files in data/raw/qld")
            print("- Extract sectional times data")
            print("- Apply QLD-specific column filtering")
            print("- Save CSV files to format-specific folders in data/processed/qld")
            print("  (pj/, triples/, triples_detailed/, unknown/)")
            print("- Skip files that already exist (to resume interrupted runs)")
            return
    
    print("QLD PDF Batch Processor")
    print("Starting batch processing of all QLD PDFs...")
    print("Use --debug flag to see detailed error messages")
    print("")
    
    process_all_qld_pdfs()
